...
def main():
    ...
    # 6) Save Top 10 lists (include useful context)
    ...
    cols30 = keep_cols + ["prob_hit30"]
    top30 = top30[[c for c in cols30 if c in top30.columns]].copy()
    top30.to_csv(OUT_TOP10_30, index=False)

    top43 = today.sort_values("prob_hit4threes", ascending=False).head(10)
    cols43 = keep_cols + ["prob_hit4threes"]
    top43 = top43[[c for c in cols43 if c in top43.columns]].copy()
    top43.to_csv(OUT_TOP10_4THREES, index=False)

    logging.info(f"✅ Wrote {OUT_TOP10_30} and {OUT_TOP10_4THREES}")
